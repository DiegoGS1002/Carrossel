...
// Iniciando as variaveis que mudaram seu status.
...
const createcarrossel = () => {
  ...
  const length = carrosselItems.length; // Longitude do array
...
// Função que da suavidade a animação
const lerp = (a, b, n) => {
  return n * (a - b) + b;
};
...
const distanceZ = (widthElement, length, gap) => {
  return widthElement / 2 / Math.tan(Math.PI / length) + gap; // Distancia Z dos items
};

// Calcule a altura do contêiner usando o campo de visão e a distância da perspectiva
...
const onResize = () => {
  // Obtiem a propiedade do tamanho do carrossel
